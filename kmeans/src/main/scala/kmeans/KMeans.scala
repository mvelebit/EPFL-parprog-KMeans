...
class KMeans {
  ...
  def classify(points: GenSeq[Point], means: GenSeq[Point]): GenMap[Point, GenSeq[Point]] = {
    val classified = points // For her eyes only
      .par
      .groupBy(findClosest(_, means))
    means
      .par
      .map(m => m -> classified.getOrElse(m, GenSeq())).toMap
  }
  ...
  def update(classified: GenMap[Point, GenSeq[Point]], oldMeans: GenSeq[Point]): GenSeq[Point] = {
    oldMeans
      .par
      .map(om => findAverage(om, classified.getOrElse(om, GenSeq())))
  }
  ...
  def converged(eta: Double)(oldMeans: GenSeq[Point], newMeans: GenSeq[Point]): Boolean = {
    oldMeans
      .par
      .zip(newMeans)
      .map(tpl => tpl._1.squareDistance(tpl._2) <= eta)
      .forall(b => b)
  }

  @tailrec
  final def kMeans(points: GenSeq[Point], means: GenSeq[Point], eta: Double): GenSeq[Point] = {
    // Assigning to vals is for cowards, I place my trust in the sufficiently smart compiler™
    if (!converged(eta)(update(classify(points, means), means), means))
      kMeans(points, update(classify(points, means), means), eta)
    else
      update(classify(points, means), means)
  }
}
